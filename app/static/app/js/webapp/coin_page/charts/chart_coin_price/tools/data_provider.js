...
function generateHistoricalPriceData(basePrice, years = 4) {
    // Округляем до 9 знаков
    let currentPrice = parseFloat(parseFloat(basePrice).toFixed(9));
    ...
    // Настройки колебаний
    const maxDailyChangePercent = 0.05;  // максимум 5% в день
    const minDailyChangePercent = 0.001; // минимум 0.1% в день

    // Текущая дата
    const today = new Date();

    // Добавляем сегодняшнюю цену (текущая)
    result.push({
        time: today.toISOString().slice(0, 10),
        value: currentPrice
    });

    // Идём назад во времени
    ...
    for (let i = 1; i < days; i++) {
        ...
        // генерим изменение в обратную сторону
        const direction = Math.random() < 0.5 ? -1 : 1;
        const percent =
            Math.random() * (maxDailyChangePercent - minDailyChangePercent) +
            minDailyChangePercent;

        // цена в прошлом = "будущая цена минус изменение"
    ...
    // сортируем по времени от старой даты к новой (если нужно)
...
export class DataProviderMock {
    ...
    // ================================================================= //
    getMockData() {
        return JSON.parse(document.getElementById('chart-data-price-json').textContent);
    }
